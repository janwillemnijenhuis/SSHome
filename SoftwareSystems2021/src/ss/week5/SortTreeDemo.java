...
public class SortTreeDemo {
    ...
    public TreeNode root;  // Pointer to the root node in a binary tree.
...
//    }  // end main()
     ...
     */
    public void treeList(TreeNode node, OutputStreamWriter pw) throws IOException {
        if ( node != null ) {
            treeList(node.left, pw);             // Print items in left subtree.
            pw.write(node.item + "\n");
            treeList(node.right, pw);            // Print items in the right subtree.
        }
    } // end treeList()
